...
impl<T> LinkedList<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        ...
        unsafe {
            ...
            if let Some(old) = self.front {
                ...
                // if no front, then we're an empty list and need
                // to set the back too.
                self.back = Some(new);
            }
            self.front = Some(new);
    ...
    pub fn push_back(&mut self, elem: T) {
        ...
        unsafe {
            ...
            if let Some(old) = self.back {
                // put new back before the old one
                (*old.as_ptr()).back = Some(new);
                (*new.as_ptr()).front = Some(old);
            } else {
                // if no back, then we're an empty list and need
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        unsafe {
            // Only have to do stuff here if there is a front node to pop
            // Note that we don't need to mess around with the `take` anymore
            // because everything is Copy and there are no dtors that will
            // run if we mess up... riiight?
            self.front.map(|node| {
                ...
                if let Some(new) = self.front {
                    ...
                    // if the front is null, then this list is empty
                    self.back = None
                }

                self.len -= 1;
                result
                // Box gets freed here, knows there is no T.
            })
        }
    }
    ...
    pub fn pop_back(&mut self) -> Option<T> {
        unsafe {
            // Only have to do stuff here if there is a back node to pop
            // Note that we don't need to mess around with the `take` anymore
            // because everything is Copy and there are no dtors that will
            // run if we mess up... riiight?
            self.back.map(|node| {
...
impl<'a, T> CursorMut<'a, T> {
    ...
    pub fn move_next(&mut self) {
        if let Some(cur) = self.cur {
            ...
            // We're at the ghost, but it's the only element... do nothing
        }
    }
    ...
    pub fn splice_before(&mut self, mut input: LinkedList<T>) {
        unsafe {
            // we can either `take` the input's ptrs or `mem::forget`
...
/// ```compile_fail
/// use many_linked_lists::sixth::IterMut;
